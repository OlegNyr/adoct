...
## Каталог тулов (75) + промпт

**Jira — задачи:** `jira_get_issue`, `jira_search` (JQL), `jira_get_transitions`,
`jira_create_issue` (все поля за один вызов: assignee/labels/epicKey/links/кастомные поля),
`jira_update_issue`, `jira_transition_issue`, `jira_delete_issue`,
...
**Jira — связи / наблюдатели / worklog:** `jira_get_link_types`, `jira_link_issues`
(внутренняя связь по имени типа), `jira_create_issue_link`,
...
`jira_assign_issue`. Экран также показывает URL для подключения и статус сервера (запущен/остановлен), а
**галками групп инструментов** (Jira / Confluence / Bitbucket) можно скрыть ненужные тулы — выключенная
группа не отдаётся по `tools/list`. Изменения настроек перезапускают сервер и сразу видны тулам.
...
- Native (GraalVM): `./gradlew :adoct-mcp-cli:nativeCompile --no-configuration-cache` (вход
  `McpCliNative` → `ToolRegistry.coreTools()`, **74 тула** без asciidoctorj/JRuby-зависимого
...
| Кол-во тулов | **75** (42 Jira + 23 Confluence + 10 Bitbucket) | **~72** (52 Jira + 20 Confluence) | много (Jira/Confluence/JSM/Bitbucket/Compass) |
