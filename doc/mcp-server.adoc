...
* *Шаблоны задач* — таблица имя / *свободный текст*; `jira_list_templates` возвращает их как есть, а
  вызов `jira_create_issue` ассистент собирает сам по тексту шаблона (сервер шаблоны не интерпретирует).
* *Состояния задач* — диаграмма *PlantUML state*; `jira_get_workflow` отдаёт её текстом
  (плюс `jira_get_project_statuses` — живые статусы из Jira).

Назначение — `jira_assign_issue`. Изменения в настройках перезапускают сервер и сразу видны тулам.
...
(`MCP_HOST`/`MCP_TOKEN` — быстрый одно-эндпоинтный режим, плюс `MCP_PORT`, `MCP_BIND`, `MCP_TRANSPORT`,
`MCP_JIRA_PROJECT`, `MCP_CONFLUENCE_SPACE`), аргументы CLI. Файл задаёт `endpoints`/`team`/`templates`/`workflowDiagram`.
